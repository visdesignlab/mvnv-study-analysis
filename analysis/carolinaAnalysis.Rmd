...
experience <- mutate(filteredFrame, visExperience = ifelse(as.numeric( as.character(value)) <3, "low",
                                     ifelse(as.numeric( as.character(value))<6, "medium", "high"))) 
...
newFrame <- merge(newFrame,filter(dfp,measure=="sex"), by="prolificId")
...
ggplot(newFrame, aes(x = visType, y = avgAcc)) +
    geom_point(aes(color=visType,alpha=0.5,size=4))+
    facet_grid(value ~ visExperience) +
