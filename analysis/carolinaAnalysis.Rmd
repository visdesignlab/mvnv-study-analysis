...
number = as.character(sdf[1,]$taskNumber)
 ...
 data$value <- factor(data$value, levels = unique(data$value[order(-data$n)]))
  ...
  ggsave(paste("charts/taskAnswers/",task,".pdf",sep=""), width=10, height=3, units="in")
...
ggsave("charts/insights.pdf", width = 7, height =5, units = "in")
...
ggsave(paste("charts/learning_effects.pdf", sep=""), width = 6, height =6, units = "in")
...
dfv %>% 
  ...
  coord_flip() +
     facet_grid( hypothesis_1 ~ label, scales='free')+
    theme(strip.text.y = element_text(angle=0))+
   xlab ("") + 
    ylab("")+
...
plotData <-df %>% 
  ...
  coord_flip() + 
    ...
    theme ( plot.caption = element_text(hjust = 0.5, vjust =5,  face = "italic")) + 
    xlab ("") + 
    ylab("")+
    scale_fill_manual(values=visType.colors)+
    scale_color_manual(values=visType.colors)+      
...
ggsave(paste("violins/",gsub(" ", "", paste(number,tt), fixed = TRUE),".pdf", sep=""), width = 2.5, height = 2.5, units = "in")
