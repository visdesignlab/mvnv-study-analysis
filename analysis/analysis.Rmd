...
age<- filter(dfp,measure=="age")
acc<- filter(dfp,measure=="averageAccuracy")

value<- filter(dfp,measure== "age")+
  mutate( value = as.numeric( as.character(value)) ) %>% 
...
dfp %>% 
  filter(measure == "browser") %>% 
  mutate( value =  as.character(value))  %>% 
  ggplot(aes(x="browser", y=value)) +
    #geom_beeswarm(priority='random',cex=2, alpha=0.6) +
  coord_flip() +
  xlab("") + ylab("")
...
df %>% 
  ...
  mutate( value = as.numeric( as.character(value)) ) %>% 
  ggplot(aes(x=taskId, y=value, fill=visType, color=visType)) +
